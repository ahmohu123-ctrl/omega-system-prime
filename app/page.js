...
export default function AhmohuSystem() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      const aiMsg = { role: 'ai', content: `تم تحليل طلبك [${input}] بنجاح. نظام AHMOHU في خدمتك.` };
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans flex flex-col items-center relative overflow-hidden px-4">
      ...
      {/* الخلفية المتحركة */}
      <div className="bg-grid opacity-20 pointer-events-none absolute inset-0"></div>
      ...
      {/* الهوية البصرية - الأسطوانة والاسم */}
      <div className="flex flex-col items-center justify-center py-10 z-10 w-full">
      ...
      {/* منطقة الشات */}
      <main className="w-full max-w-2xl flex-1 overflow-y-auto z-10 space-y-4 py-4 scrollbar-hide">
        <AnimatePresence>
          {messages.map((msg, i) => (
            <motion.div
              key={i}
        ...
        {isTyping && <div className="text-[10px] text-cyan-500 animate-pulse uppercase tracking-widest px-2">Processing...</div>}
        <div ref={chatEndRef} />
      </main>

      {/* شريط الإدخال */}
      <footer className="w-full max-w-2xl pb-8 pt-4 z-20 bg-black">
        <form onSubmit={handleSubmit} className="input-glass rounded-full p-2 flex items-center relative overflow-hidden border border-white/10">
          ...
          />
          <button type="submit" className="p-3 bg-cyan-500 rounded-full text-black hover:bg-cyan-400 transition-all active:scale-90">
            <Send size={18} />
          </button>
        </form>
      </footer>
